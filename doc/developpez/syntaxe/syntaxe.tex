\part{Syntaxe et éléments de bases de Nit}
...
Un identificateur ne peut pas appartenir à la liste des mots clés réservé du langage Nit :
\begin{multicols}{4}
  \begin{itemize}
      \item |abort|
      \item |abstract|
...
\begin{itemize}
    ...
    \item L'indentation est faite avec le caractère de tabulation et est afficher comme 8 espaces ;
...
\chapter{Éléments de bases de Nit}
...
Pour éviter les exceptions |nullPointerException| qui peuvent être très fréquentes sur d'autres langages, Nit impose au développeur de préciser si une variable a le droit de prendre la valeur |null|.
...
Comme les autres langages de développement, Nit propose un ensemble d'instruction qui permettent d'organiser et de structurer les traitements. L'usage de ces instructions est similaire à celui rencontré à leur équivalent dans d'autres langages.
...
\subsubsection{Loop}
Les boucles infinies sont souvent utilisés accompagnées de l'instruction |break|. Elles sont utiles pour implémenter les boucles de type \og jusqu'à \fg |untill| ou pour simuler le |exit when| de Ada.
...
Attention : Si on utilise un |else| ou un |else if|, l'instruction ne peut pas être écrit \og OnLiner \fg.
...
L'instruction |assert| vérifie si une expression booléenne est vrai, sinon elle arrêtera le programme comme |abort|. Elle peut être combinée à une close |else| pour exécuter du code avant d'arrêter le programme :
...
En Nit, il n'existe pas de mot clé de transtypage. Les conversions de types se font par des méthodes. La bibliothèque standard fournit une série de classe qui contiennent des méthodes de manipulation et de conversion de types.
...
Par exemple la classe Int propose les méthodes suivantes :
\begin{description}
...
Dans un chaîne de caractères, plusieurs caractères particuliers doivent être échappés grâce au caractère |\| pour être utilisés. 
...
Une notation allégée permet de concaténer une variable ou une expression avec une chaine de caractère :
...
En Nit il est possible de comparer deux chaînes de caractères directement grâce à l'opérateur |==|. Contrairement à Java par exemple, Nit va comparer la valeur des objets String est non pas les objets en eux-mêmes.
\lstinputlisting[language=Nit]{./syntaxe/listings/string2_c.nit}

\subsection{Autres opérations sur les chaînes de caractères}
Les méthodes |to_upper| et |to_lower| permettent respectivement d'obtenir une chaîne tout en majuscule ou tout en minuscule.
...
Les instructions de contrôle de flot vont eux aussi agir sur le typage de la variable :
