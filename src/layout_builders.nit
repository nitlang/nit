...
abstract class TypingBMizer[E: Object]
	...
	private fun build_poset(elements: Set[E]) is abstract
	private var poset: nullable POSet[E]
end
...
class MClassColorer
	...
	redef fun build_poset(elements) do self.poset = mmodule.flatten_mclass_hierarchy
end
...
abstract class MPropertyColorer[E: MProperty]
	...
	private fun colorize(mclasses: Set[MClass]): Map[E, Int] do
		self.class_colorer.build_poset(mclasses)
		self.class_colorer.tag_elements(mclasses)
		self.class_colorer.build_conflicts_graph
	...
	private fun colorize_core(mclasses: Set[MClass]) do
		...
		for mclass in mclasses do
			...
			if mclass.in_hierarchy(mmodule).direct_greaters.length == 0 then
