...
class DepWeighter
	...
	redef fun weight_mmodule(mmodule) do
		mmodule.weight = mmodule.in_importation.smallers.length.to_f
	end

	redef fun weight_mclassdef(mclassdef) do
		mclassdef.weight = mclassdef.mclass.in_hierarchy(mainmodule).smallers.length.to_f
	end
