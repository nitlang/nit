...
fprintf(stderr, " (%s:%d)\n", "src/common_ffi/extern_classes.nit", 22);
exit(1);
}
var = var1;
RET_LABEL:;
...
fprintf(stderr, " (%s:%d)\n", "src/common_ffi/extern_classes.nit", 24);
exit(1);
}
var = var1;
RET_LABEL:;
...
fprintf(stderr, " (%s:%d)\n", "src/common_ffi/extern_classes.nit", 44);
exit(1);
} else {
...
if (var1 == NULL) {
fprintf(stderr, "Runtime error: %s", "Reciever is null");
fprintf(stderr, " (%s:%d)\n", "src/common_ffi/extern_classes.nit", 94);
exit(1);
} else {
...
CHECK_NEW_ffi_base__ForeignType(var13);
