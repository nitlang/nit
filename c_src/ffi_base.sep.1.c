...
fprintf(stderr, " (%s:%d)\n", "src/common_ffi/ffi_base.nit", 102);
exit(1);
RET_LABEL:;
return var;
}
...
fprintf(stderr, " (%s:%d)\n", "src/common_ffi/ffi_base.nit", 105);
exit(1);
RET_LABEL:;
}
...
fprintf(stderr, " (%s:%d)\n", "src/common_ffi/ffi_base.nit", 106);
exit(1);
RET_LABEL:;
}
...
fprintf(stderr, " (%s:%d)\n", "src/common_ffi/ffi_base.nit", 108);
exit(1);
RET_LABEL:;
}
...
fprintf(stderr, " (%s:%d)\n", "src/common_ffi/ffi_base.nit", 117);
exit(1);
}
...
if (!var4) {
fprintf(stderr, "Runtime error: %s", "Assert failed");
fprintf(stderr, " (%s:%d)\n", "src/common_ffi/ffi_base.nit", 125);
exit(1);
}
...
fprintf(stderr, " (%s:%d)\n", "src/common_ffi/ffi_base.nit", 135);
exit(1);
RET_LABEL:;
return var;
}
...
fprintf(stderr, " (%s:%d)\n", "src/common_ffi/ffi_base.nit", 136);
exit(1);
RET_LABEL:;
return var;
}
...
if (var7 == NULL) {
fprintf(stderr, "Runtime error: %s", "Reciever is null");
fprintf(stderr, " (%s:%d)\n", "src/common_ffi/ffi_base.nit", 179);
exit(1);
} else {
var8 = ((val* (*)(val*))(var7->class->vft[COLOR_model_base__MModule__name]))(var7) /* name on <var7:nullable MModule>*/;
}
