...
module opengl is pkgconfig("x11", "egl", "gl")
...
extern class Opengles1DrawableImage in "C" `{struct mnit_opengles_DrawableTexture*`}
    ...
    new ( w, h: Int ) is extern `{
		...
		/* depth */
		glGenRenderbuffers(1, &image->depth);
		glBindRenderbuffer(GL_RENDERBUFFER, image->depth);
		...
		/* tex framebuffer */
		glGenRenderbuffers(1, &image->color);
		glBindRenderbuffer(GL_RENDERBUFFER, image->color);
		glRenderbufferStorage(GL_RENDERBUFFER, GL_RGBA8, w, h);
		glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_RENDERBUFFER, image->color );

		if ( glCheckFramebufferStatus( GL_FRAMEBUFFER ) != GL_FRAMEBUFFER_COMPLETE )
		{
			LOGW("framebuffer not set", glCheckFramebufferStatus(GL_FRAMEBUFFER));
			exit(1);
		}
    ...
    fun set_as_target is extern `{
		LOGI( "sat %i", recv->fbo );
		glBindFramebuffer(GL_FRAMEBUFFER, recv->fbo);
		if (glGetError() != GL_NO_ERROR) LOGW("gl error", 0);
		/*glGetIntegerv(GL_FRAMEBUFFER_BINDING,&recv->fbo);
		if (glGetError() != GL_NO_ERROR) LOGW( "gl error a");*/
		glViewport(0, 0, recv->super.width, recv->super.height);

		glMatrixMode(GL_PROJECTION);
		glLoadIdentity();
		glOrtho(0.0f, recv->super.width, recv->super.height, 0.0f, 0.0f, 1.0f);
		glMatrixMode(GL_MODELVIEW);
		glFrontFace( GL_CW );
