...
class Lights
	var grid: Array[Array[Bool]] is noinit
	var width: Int
	var height: Int

	init do
		grid = new Array[Array[Bool]].with_capacity(width)
		for i in [0 .. width[ do
			grid.add(new Array[Bool].with_capacity(height))
			var curr_grid = grid[i]
			for j in [0 .. height[ do curr_grid.add false
		end
	end
...
if args.is_empty then
	print "Usage: ./fire_hazard input.txt"
	exit 1
end
